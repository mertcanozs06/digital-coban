...
export const applyPartner = async (req, res) => {
  const { role, company_name, tax_number, contact_name, email, phone, address, message } = req.body;
  ...
  try {
    ...
    await pool.request()
      ...
      .query(`
        INSERT INTO PartnerApplications (role, company_name, tax_number, contact_name, email, phone, address, message)
        VALUES (@role, @company_name, @tax_number, @contact_name, @email, @phone, @address, @message)
      `);
